...
VALUES (?, ?, ?, ?, ?, ?, ?) RETURNING student_roll_number, semester, subject_code, grade, sub_gp, subject_name, subject_credits
`
...
const getRanksData = `-- name: GetRanksData :many
SELECT
    student.roll_number,
...
type GetRanksDataParams struct {
	Batch  string `json:"batch"`
	Branch string `json:"branch"`
}

type GetRanksDataRow struct {
	RollNumber string  `json:"roll_number"`
	CurrCg     float64 `json:"curr_cg"`
}

func (q *Queries) GetRanksData(ctx context.Context, arg GetRanksDataParams) ([]GetRanksDataRow, error) {
	rows, err := q.db.QueryContext(ctx, getRanksData, arg.Batch, arg.Branch)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []GetRanksDataRow
	for rows.Next() {
		var i GetRanksDataRow
		if err := rows.Scan(&i.RollNumber, &i.CurrCg); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
...
SELECT student_roll_number, semester, subject_code, grade, sub_gp, subject_name, subject_credits FROM subject_result_data where student_roll_number = ? and semester = ?
`
